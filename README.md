...
* **Auto-Broadcast vs. Explicit Hints:** While the Catalyst Optimizer will automatically attempt to broadcast any dataframe below the 10MB threshold without explicit code, relying purely on this auto-behavior in production is an engineering risk. If background dataset statistics become stale, Spark may miscalculate the dataframe size and drop back to a Sort-Merge join. Explicitly wrapping a dataframe in `broadcast()` forces a hard override, guaranteeing optimization regardless of metadata delays.
